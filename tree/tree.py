...
#################################################################
##                                                             ##
## MyPAF - The My Purpose Analysis Framework                   ##
...
## defining new tree structure
maxelm = 10
for v, var in enumerate(cfg.vars):
	if var[1] == "AD":
		eval(var[0] + " = array('d', maxelm*[0.])")
	if var[1] == "AF":
		eval(var[0] + " = array('f', maxelm*[0.])")
	if var[1] == "AI":
		eval(var[0] + " = array('i', maxelm*[0.])")
	if var[1] == "D":
		eval(var[0] + " = double(0.)")
	if var[1] == "F":
		eval(var[0] + " = float(0.)")
...
for s, sample in enumerate(cfg.samples):

	vb.sample(sample)

	f = ROOT.TFile(cfg.path + cfg.sample, "read")
	t = f.Get(cfg.tree)
